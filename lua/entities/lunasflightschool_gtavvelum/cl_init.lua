...
function ENT:AnimLandingGear()
	...
	self.SMRG = self.SMRG and self.SMRG + (83 *  (1 - self:GetLGear()) - self.SMRG) * FrameTime() * 2 or 0
	
	self.SFGD = self.SFGD and self.SFGD + (90 *  (1 - self:GetLGear()) - self.SFGD) * FrameTime() * 2 or 0
	--IJWSBDFIJDSBFOJNDSFJON
	self:ManipulateBoneAngles( 10, Angle( -self.SFGD,0  ) )
	self:ManipulateBoneAngles( 11, Angle( self.SFGD,0  ) )
	self:ManipulateBoneAngles( 9, Angle( 0,0,-self.SMLG  ) )
	--Wing Wheels
	self:ManipulateBoneAngles( 7, Angle( self.SMRG,0  ) )
	self:ManipulateBoneAngles( 8, Angle( -self.SMRG,0,0  ) )
	--Wing Doors
	self:ManipulateBoneAngles( 5, Angle( -self.SMLG,0  ) )
	self:ManipulateBoneAngles( 6, Angle( self.SMLG,0,0  ) )
