...
if SERVER then
	...
	function ENT:Initialize()
		...
		local nakconvar = GetConVarNumber( "lfs_giblifetime" )
		self.DieTime = CurTime() + nakconvar
		...
		for _, v in pairs( self.GibModels ) do
			...
			if IsValid( ent ) then
				...
				if IsValid( PhysObj ) then
					local nakconvar2 = GetConVarNumber( "lfs_nogibexplosion" )
					local nakconvar3 = GetConVarNumber( "lfs_nogibexplosioncustomforcevalue" )
					if nakconvar2 == 0 then
						...
						PhysObj:SetVelocityInstantaneous( VectorRand() * nakconvar3 )
						
						PhysObj:AddAngleVelocity( VectorRand() * nakconvar3/1.2 ) 
