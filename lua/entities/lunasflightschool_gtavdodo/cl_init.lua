		...
		for _, v in pairs(Pos) do 
			if math.random(0,1) == 1 then
				local effectdata = EffectData()
					...
					effectdata:SetAngles( Angle(-110,0,0) )
...
function ENT:AnimFins()
	...
	self:ManipulateBoneAngles( 9, Angle( -self.smYaw) )
...
function ENT:AnimRotor()
	...
	Rot:Normalize() 
	self:ManipulateBoneAngles( 11, Rot )
...
function ENT:AnimLandingGear()
	self.SMLG = self.SMLG and self.SMLG + (155 * (1 - self:GetRGear()) - self.SMLG) * FrameTime() * 4 or 0 
	self.SMRG = self.SMRG and self.SMRG + (90 *  (1 - self:GetLGear()) - self.SMRG) * FrameTime() * 2 or 0
	
	self.SFGD = self.SFGD and self.SFGD + (90 *  (1 - self:GetLGear()) - self.SFGD) * FrameTime() * 2 or 0
	--IJWSBDFIJDSBFOJNDSFJON

	--Wing Wheels
	self:ManipulateBoneAngles( 7, Angle( 0,0,-self.SMLG  ) )
	self:ManipulateBoneAngles( 8, Angle( 0,0,-self.SMLG  ) )
	--Wing Doors
	self:ManipulateBoneAngles( 5, Angle( 0,0,-self.SMRG  ) )
...
function ENT:Draw()
	...
	for i = 0,1 do
		...
		local pos = self:LocalToWorld( Vector(36.29,247.088 * Sub,72.592) )
